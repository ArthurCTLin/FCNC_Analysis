{
    TFile *fin1 = new TFile("./sample/2016_Zut//hist_w_mt_eee.root");
    TFile *fin2 = new TFile("./sample/2016_Zut//hist_b_no_eee.root");
    ...
    TFile *fin4 = new TFile("./sample/2016_Zut//hist_Zmass_eee.root");
    TH1D *TTCR_sig = (TH1D*)fin3->Get("ttbar");
    TH1D *TTCR_sig_eee = (TH1D*)fin4->Get("ttbar");
    TH1D *TTCR_data = (TH1D*)fin3->Get("data");

    TFile *fin5 = new TFile("./sample/2016_Zut//hist_mva_eee.root");
    //
...
//{{{ WZCR   
    ...
    TCanvas *c2 = new TCanvas("c2", "Transverse Mass of W", 10,10,600,550);
    frame->SetMaximum(frame->GetMaximum()*1.5);
    frame->Draw();
    RooFitResult *result = WZCR_total_pdf.fitTo(bin_WZCR_Data, ExternalConstraints(gausConstraint), Save(true), Extended(true), Minos(true), SumW2Error(kTRUE));
...
//{{{TTCR
    ...
    double ttbar_No = (TTCR_N_sig.getVal()-sigregion_sig2->getVal()*TTCR_N_sig.getVal())*(TTCR_sig_eee->Integral()/TTCR_sig->Integral())*(Exp_SR/Exp_SB);
...
//}}}
    ...
    //if(Final_bkg_ttbar->Integral()<0){
    //    for(int i=0 ; i<Final_N_bkg_ttbar->GetNbinsX() ; i++){
    //        Final_bkg_ttbar->SetBinContent(i+1, 0.);
    //        Final_N_ttbar = 0;
    //    }
    //}
    
    RooRealVar Final_N_sig("Final_N_sig", "Final_N_sig", 10, -50., 300);   
    ...
    RooArgSet Final_gausConstraint(Final_constraint_ttbar, Final_constraint_TTZ, Final_constraint_WZ, Final_constraint_ZZ, Final_constraint_tZq, Final_constraint_other);
    ...
    RooAddPdf Final_total_pdf("Final_total_pdf", "Final_total_pdf", RooArgList(Final_sig_pdf, Final_bkg_pdf_ttbar, Final_bkg_pdf_TTZ, Final_bkg_pdf_WZ, Final_bkg_pdf_ZZ, Final_bkg_pdf_tZq, Final_bkg_pdf_other), RooArgList(Final_N_sig, Final_N_bkg_ttbar, Final_N_bkg_TTZ, Final_N_bkg_WZ, Final_N_bkg_ZZ, Final_N_bkg_tZq, Final_N_bkg_other));
